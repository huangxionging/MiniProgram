...
Page({
  ...
  getHomePage: function() {
    ...
    baseNetLinkTool.getRemoteDataFromServer("homePage", "获取首页评测信息").then(res => {
      // 无评测
      baseTool.print(res)
  ...
  endClick: function(e) {
    ...
    baseTool.showAlertInfoWithCallBack({
      ...
      if (res.type == 0) {
        baseNetLinkTool.getRemoteDataFromServer("finishedGame", "结束评测", {
  ...
  createContest: function() {
    // 完善诊所信息
    loginManager.completeClinicInfo("创建评测").then(res => {
      ...
      baseNetLinkTool.getRemoteDataFromServer("addGame", "创建评测", {
        clinicId: baseNetLinkTool.getClinicId()
      }).then(res => {
        wx.startPullDownRefresh()
        baseTool.showAlertInfoWithCallBack({
          title: "评测创建成功",
          content: "请按右上角“实时投屏”- 将分享页投至屏幕，评测者即可扫码报名",
          confirmText: "我知道了"
        }, (res) => {})
      }).catch(res => {
        baseNetLinkTool.showNetWorkingError(res)
