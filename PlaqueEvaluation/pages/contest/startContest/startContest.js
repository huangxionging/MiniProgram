...
Page({
    ...
    baseNetLinkTool.getRemoteDataFromServer("getPlayers", "绑定设备", {
      ...
      if (papPlayerList) {
        ...
        rowDataArray.sort((a, b) => {
          if (b.plaqueLevel != a.plaqueLevel) {
            return b.plaqueLevel - a.plaqueLevel
          } else {
            return 0;
          }
        })
  ...
  modifyNameClick: function(e) {
    baseTool.print(e)
    let that = this
    that.data.currentItem = e.detail
    that.setData({
      isModifyName: true,
      currentName: that.data.papSectionDataArray[e.detail.indexPath.section].rowDataArray[e.detail.indexPath.row].name
    })
  },

  getModifyInput: function(e) {
    let that = this
    if (e.detail.value.length == 0) {
      return;
    }
    that.data.currentName = e.detail.value
  },
  cancleClick: function(e) {
    let that = this
    that.data.currentItem = {}
    that.setData({
      isModifyName: false,
      currentName: '',
      currentTelephone: ''
  ...
  getModifyTel: function(e) {
    let that = this
    if (e.detail.value.length == 0) {
      return;
    }
    that.data.currentTelephone = e.detail.value
  },
  saveNameClick: function(e) {
    let that = this
    // that.setData({
    //   isModifyName: false,
    //   currentName: ''
    ...
    baseNetLinkTool.getRemoteDataFromServer("updatePlayerName", "更改评测者姓名", {
      name: that.data.currentName,
      playerId: that.data.currentItem.playerId,
      telephone: that.data.currentTelephone
    }).then(res => {
      let that = this
      that.data.currentItem = {}
      that.setData({
