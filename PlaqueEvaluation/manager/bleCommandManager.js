...
function findDeviceCommand() {
  ...
  let brushMethod = '01'
  // 获得格式化时间数组
  let systemTimeArray = getFormatDateArray()
  // 转换成16进制字符串
  let systemTimeHex = baseHexConvertTool.arrayToHexString(systemTimeArray)
  // 生成完整的命令字符串
  let commandHexString = header + brushMethod + headHabit + systemTimeHex
  baseTool.print([systemTimeHex, '时间'])
  // 生成命令数组
  let buffer = baseHexConvertTool.hexStringToArrayBuffer(commandHexString)
  return buffer
}
...
function connectReplyDeviceCommand(brushMethod = '01') {
  ...
  let systemTimeHex = baseHexConvertTool.arrayToHexString(systemTimeArray)
  // 生成完整的命令字符串
  let commandHexString = header + systemTimeHex + headHabit + brushMethod + '00'
  baseTool.print([systemTimeHex, '时间'])
  // 生成命令数组
  let buffer = baseHexConvertTool.hexStringToArrayBuffer(commandHexString)
  return buffer
...
function onceDataEndReplyDeviceCommand() {
  let header = 'f103f8'
  // 生成命令数组
  let buffer = baseHexConvertTool.hexStringToArrayBuffer(header)
  return buffer
}
...
function dataBoxCommand(array, itemObject) { //构造函数模式
  let that = this;
  let data = {};
  ...
  let brushTimeUnionHead = Number(array[3]) + "" + Number(array[4]) + '-' + month + '-' + day
  console.log(brushTimeUnionHead)
  let brushTimeUnion = [Number(array[7]), Number(array[8]), Number(array[9])].map(formatNumber).join(':')
  ...
  data.playerName = itemObject.name
  data.playerId = itemObject.playerId
